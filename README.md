[![image charts rust library logo](./logo.jpg)](#getting-started)

[![Crates.io](https://img.shields.io/crates/v/image_charts.svg)](https://crates.io/crates/image_charts)
[![Documentation](https://docs.rs/image_charts/badge.svg)](https://docs.rs/image_charts)
[![License](https://img.shields.io/crates/l/image_charts.svg)](LICENSE)

![Libraries.io dependency status for GitHub repo](https://img.shields.io/librariesio/github/image-charts/rust)
...
# Blocking only
image_charts = { version = "6", default-features = false, features = ["blocking"] }

# Both async and blocking
image_charts = { version = "6", features = ["full"] }
```
...
fn main() {
    ...
    let url = pie.to_url();
    println!("{}", url);
    // https://image-charts.com/chart?chd=a%3A2.5%2C5%2C8.3&chs=600x300&cht=p
}
```
...
<a name="to_buffer"></a>
#### `to_buffer()` -> `Result<Vec<u8>, ImageChartsError>` (async)
#### `to_buffer_blocking()` -> `Result<Vec<u8>, ImageChartsError>` (blocking)
...
<a name="to_file"></a>
#### `to_file(path)` -> `Result<(), ImageChartsError>` (async)
#### `to_file_blocking(path)` -> `Result<(), ImageChartsError>` (blocking)
...
#### `to_data_uri()` -> `Result<String, ImageChartsError>` (async)
#### `to_data_uri_blocking()` -> `Result<String, ImageChartsError>` (blocking)
...
fn main() {
    ...
    println!("{}", url);
    // https://custom-domain.tld/chart?chd=t%3A60%2C40&chf=...&ichm=...
}
```
